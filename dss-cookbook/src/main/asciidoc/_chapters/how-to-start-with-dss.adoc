...
* Java 17 or higher (tested up to Java 26) is required;
...
By default, DSS packages DSS Standalone for Windows x64 OS only. Should you need to create a version for Linux OS, please use the profile below:

.Maven command to build DSS Standalone package for Linux
----
mvn clean install -P linux
----

Download links for the DSS Standalone Application:
...
* Linux:
** https://ec.europa.eu/digital-building-blocks/DSS/webapp-demo/downloads/dss-app-minimal-linux.tar.gz[Minimal TAR.GZ (application + .sh file)];
** https://ec.europa.eu/digital-building-blocks/DSS/webapp-demo/downloads/dss-app-complete-linux.tar.gz[Complete TAR.GZ (application + .sh file + OpenJDK + JavaFX SDK)].
...
If you continue with a default `war` packaging option, you may benefit from `dss-demo-bundle` module, encapsulating the created package within a Tomcat 11 bundle. After a successful build, in the directory `/dss-demo-bundle/target/` you will be able to find two containers: `dss-demo-bundle.zip` and `dss-demo-bundle.tar.gz`. Despite the different container type, the content of both containers is the same. After extracting the content, you will need to run the file `Webapp-Startup.bat` in order to launch the server and the file `Webapp-Shutdown.bat` to stop the server. After running the server, the web-application will be available at the address `http://localhost:8080/`.

NOTE: By default, the `dss-demo-bundle` module will create a bundle with embedded JDK 26. For other possibilities, please see options below in this section.
...
DSS webapp version with JDK 21 can be created with a command below:
...
DSS webapp version with JDK 25 can be created with a command below:

.Maven command to create a JDK 25 bundle
----
mvn clean install -P java25
----
...
specs-trusted-list:: ETSI TS 119 612 XSD schema for parsing Trusted Lists compatible with the newest version of the standard.
specs-trusted-list-v211:: ETSI TS 119 612 XSD schema for parsing Trusted Lists compatible with v2.1.1 of the standard.
...
specs-lote-xml:: ETSI TS 119 602 XML schema for List of Trusted Entities (<<R33>>).
...
specs-lote-json:: ETSI TS 119 602 JSON schema for List of Trusted Entities (<<R33>>).
...
'''
dss-validation-job:: Generic module for performing a document validation, implements cache management and business logic of validation job.
dss-tsl-validation:: Module which allows downloading / parsing / validating of LOTL and TLs (ETSI TS 119 612).
dss-lote-validation:: Module containing generic interfaces for validating of LoTE (ETSI TS 119 602).
dss-lote-validation-json:: Processes LoTE in the JSON format.
dss-lote-validation-xml:: Processes LoTE in the XML format.
...
dss-cb-ades:: Implementation of the CB-AdES signature, augmentation and validation.
...
===== Electronic Attestations of Attributes (EAA) modules

dss-eaa-common:: Common code and interfaces for EAA.
dss-eaa-sd-jwt:: Code for creation and validation of EAAs based on JWTs (i.e. SD-JWT VC, etc.).
dss-eaa-mdoc:: Code for creation and validation of EAAs based on mdoc specification as per ISO 18013-5.
dss-eaa-revocation-common:: Common code and interfaces for EAA revocation.
dss-eaa-revocation-jwt:: Code for the validation of EAA based on JWT tokens.
dss-eaa-revocation-cwt:: Code for the validation of EAA based on CWT tokens.
...
'''
dss-eaa-creation-dto:: Data Transfer Objects used for EAA and/or EAAP creation.
dss-eaa-creation-remote:: Common classes between dss-eaa-creation-rest and dss-eaa-creation-soap.
dss-eaa-creation-rest-client:: Client for the REST EAA and/or EAAP creation webservice.
dss-eaa-creation-soap-client:: Client for the SOAP EAA and/or EAAP creation webservice.
dss-eaa-creation-rest:: REST webservice to create EAA and/or EAAP.
dss-eaa-creation-soap:: SOAP webservice to create EAA and/or EAAP.

'''
dss-eaa-validation-dto:: Data Transfer Objects used for EAA validation (REST and SOAP).
dss-eaa-validation-common:: Common classes between dss-eaa-validation-rest and dss-eaa-validation-soap.
dss-eaa-validation-rest-client:: Client for the REST EAA validation webservices.
dss-eaa-validation-soap-client:: Client for the SOAP EAA validation webservices.
dss-eaa-validation-rest:: REST webservices to validate an EAA.
dss-eaa-validation-soap:: SOAP webservices to validate an EAA.
